...
unsigned int Input::get() {
  if (_next){unsigned ch{_next}; _next=0; return ch;}
  int ch {_in.get()};
  return ch == '\x1b' ? decode(_in, ch) : ch;
}


unsigned int Input::put(unsigned int key) {
  //TODO: Using putback is causing an infinite loop when getting
  // so write to a bypass variable for now
  _next = key;
  return key;
}
